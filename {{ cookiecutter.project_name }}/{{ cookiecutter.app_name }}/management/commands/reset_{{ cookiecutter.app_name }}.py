...
class Command(BaseCommand):
    help = 'Deletes all rows from database tables backing the {{ cookiecutter.app_name }} models.'
    ...
    def handle(self, *args, **options):
        ...
        while selection.lower() not in ['y', 'n', '']:
            selection = input('This will empty all {{ cookiecutter.app_name }}-related database tables. Are you sure ([y]/n)? ')
        ...
        for model in apps.get_app_config('{{ cookiecutter.app_name }}').get_models():
        ...
        self.stdout.write(self.style.SUCCESS('Successfully cleared {{ cookiecutter.app_name }} tables'))
